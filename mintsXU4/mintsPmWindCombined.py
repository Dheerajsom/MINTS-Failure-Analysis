#!/usr/bin/env python3
"""Side-by-side wind-rose / PM animation over the same 7-day window.
...
Right: the PM time series for the same window with a moving one-hour PDF
panel hugging the shared y-axis, restyled onto the wind rose's dark teal
theme.  The PDF x-axis is static for the whole video: the global per-frame
PDF maximum is computed up front and the axis upper bound is that value
plus one.

Both panels are driven by one clock.  The wind rose shows the hour bucket
[t, t+1h); the PM window is the same forward-looking hour, so at every
...
from mintsPmWindowAnimation import field_bounds, field_label, pdf_for_window
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Side-by-side hourly wind rose and PM moving 1-hour window PDF video."
    )
    parser.add_argument("--wind-csv", type=Path, default=DEFAULT_WIND_CSV, help="WIMDA long-format CSV/CSV.GZ export.")
    parser.add_argument("--pm-dir", type=Path, default=DEFAULT_PM_DIR, help="Directory of valo_node_01_*.csv.gz daily 1s files.")
    parser.add_argument("--field", default="pm1_0", help="PM channel to animate (e.g. pm1_0, pm2_5).")
...
def load_pm_series(pm_dir: Path, field: str, start: pd.Timestamp, end: pd.Timestamp) -> pd.Series:
    """Stream one PM field out of the daily gzip files that overlap [start, end]."""
    files = sorted(glob.glob(str(Path(pm_dir) / "valo_node_01_*.csv.gz")))
    if not files:
        raise FileNotFoundError(f"No valo_node_01_*.csv.gz files found in {pm_dir}")
    ...
    for fpath in files:
        ...
        if day_end >= start and day_start <= end:
            keep.append(fpath)
    if not keep:
        raise ValueError(f"No daily files in {pm_dir} overlap {start} .. {end}")
    ...
    for fpath in keep:
        ...
        if not chunk.empty:
            frames.append(chunk)
    if not frames:
        raise ValueError(f"No rows for field '{field}' in the {len(keep)} overlapping files.")
...
def precompute_pm_frames(series: pd.Series, frames: list[AnimFrame], y_grid: np.ndarray):
...
def make_animation(
    ...
    # PM y-range over the whole 7-day window (quantile-clipped like the
    ...
    pdfs, stats_rows = precompute_pm_frames(series, frames, y_grid)
    # Static PDF axis for the whole video: the global per-frame maximum,
    # rounded up to the next 0.5 for clean ticks. (A flat +1 of padding made
    # the typical wave, whose peak is only ~0.5 density units, unreadably
    # small, so the headroom is now just the round-up margin.)
    pdf_xmax = float(np.ceil(np.nanmax(pdfs) * 2.0) / 2.0)

    label_math = (
        rf"PM$_{{{field_label_text[2:]}}}$" if field_label_text.startswith("PM") else field_label_text
    )
    ...
    fig.text(0.036, 0.868,
             f"7-day window · 1-second samples · both panels describe the same hour",
    ...
    ts_ax.set_ylabel(f"{label_math} ({UNIT_MATH})", color=INK_SECONDARY, fontsize=12)
    ...
    pdf_ax.set_xlim(pdf_xmax, 0.0)  # inverted, static for the whole video
    # A few clean ticks, skipping 0 so it cannot collide with the first
    # date label of the adjoining time-series axis.
    ticks = MaxNLocator(nbins=4, steps=[1, 2, 5, 10]).tick_values(0.0, pdf_xmax)
    pdf_ax.set_xticks([t for t in ticks if 0.0 < t <= pdf_xmax])
    ...
    # low PM = blue, high PM = red, same jet family as the rose speed classes).
    ...
    def update(index: int):
        ...
        if n:
            ...
            stats_text.set_text(
                ...
                f"n={n:,}   mean={mean_val:.2f}   std={std_val:.2f} {UNIT_MATH}"
            )
        else:
...
def main() -> None:
    ...
    print(f"Loading {args.field} from {args.pm_dir} ...")
    series = load_pm_series(args.pm_dir, args.field, window_start, window_end)
    print(f"PM points: {len(series):,} covering {series.index.min()} .. {series.index.max()}")

    out = make_animation(frames, series, out_path, args.fps, args.dpi, args.speed_unit, label)
    print(f"Saved {out}")
